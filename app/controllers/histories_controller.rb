class HistoriesController < ApplicationController
  ...
  def index
    @histories = current_user.histories.paginate(:page => params[:page], :per_page => 9)
    @history = History.new
    @support = Support.new
  ...
  def show    
    @supports = @history.supports.paginate(:page => params[:page], :per_page => 12)    
    @support = Support.new
  end
  ...
  private
    ...
    def history_params
      params.require(:history).permit(:name , :user_id)
    end
end
